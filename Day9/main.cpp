...
i128 HandleMode(i32 mode, i128 arg, Program &program) {
    i128 out;
    Array<i128> &opcodes = program.opcodes;
    switch (mode) {
        case 0: // pointer
...
i128& HandleMutableMode(i32 mode, i128 arg, Program &program) {
    i128 *out;
    Array<i128> &opcodes = program.opcodes;
    switch (mode) {
        case 0: // pointer
...
Ret RunProgram(Program &program, Array<i64> inputs) {
    ...
    for (i32 &i = program.cursor; opcodes[i] != 99;) {
        ...
        switch (op) {
            ...
            case 4: {
                i128 out = HandleMode(mode[0], args[0], program);
                i += OpCount(4);
                ret.output = out;
                ret.halt = false;
                return ret;
